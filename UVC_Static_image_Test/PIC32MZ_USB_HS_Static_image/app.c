...
__attribute__((coherent)) uint8_t packet[1024];
    ...
    switch(appData.state)
    {
        ...
        {             
            if ( data_delivered == true)
            {
                ...
                uint32_t tosend = min((sizeof(yuv_picture0)) - picture_index, 1024 - 2);               
                memcpy(packet + 2, yuv_picture0 + picture_index, tosend);  
